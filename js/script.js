...
pizzaJson.map((item, index)=>{
    ...
    pizzaItem.querySelector('a').addEventListener('click', (e)=>{
        ...
        c('.pizzaWindowArea').style.opacity = 0; //criando uma animação
        c('.pizzaWindowArea').style.display = 'flex';
        setTimeout(()=> {
            c('.pizzaWindowArea').style.opacity = 1; //mostrando a janela
        }, 200);
    });
...
function closeModal(){
    c('.pizzaWindowArea').style.opacity = 0; //criando uma animação
    setTimeout(()=> {
        c('.pizzaWindowArea').style.display = 'none'; //mostrando a janela
    }, 500);
}
